...
Item = Dict[str, Any]
RatingDict = Dict[int, float]

# ── Movie dataset (58 items) ──────────────────────────────────────────────────
...
# ── Synthetic collaborative users (same as your JS) ───────────────────────────

COLLAB_USERS: Dict[str, List[RatingDict]] = {
  "movies": [
...
def item_vector(item: Item, all_tags: List[str]) -> List[float]:
    tags = set(item["tags"])
...
def content_based(items: List[Item], ratings: RatingDict, top_n: int = 6) -> List[Item]:
    if not ratings:
        return []
    ...
    for it in items:
        mid = it["id"]
        if mid in ratings and float(ratings[mid]) >= 4.0:
...
def collaborative(items: List[Item], ratings: RatingDict,
                  users: List[RatingDict], top_n: int = 6) -> List[Item]:
    if not ratings:
        return []
    ...
    for u in users:
        ...
        if not common:
            sims.append(0.0)
            continue
        a = [ratings[iid] for iid in common]
        b = [u[iid] for iid in common]
        sims.append(cosine_sim(a, b))

    item_scores: Dict[int, float] = {}
    item_count: Dict[int, float] = {}
...
def print_items(items: List[Item]) -> None:
    for it in items:
        ...
        print(f"{it['id']:>3} | {it['emoji']}  {it['title']} ({it['year']}) [{tags_str}] – {it['subtitle']}")


def print_recommendations(recs: List[Item]) -> None:
    if not recs:
        print("No recommendations. Try rating more items.")
        return
    for i, rec in enumerate(recs, start=1):
        tags_str = ", ".join(rec["tags"])
        print(f"#{i} {rec['emoji']} {rec['title']} ({rec['year']}) [{tags_str}]")
        print(f"   Score: {rec['score']}%  |  Reason: {rec['reason']}")
        print()


def get_ratings_from_user(items: List[Item]) -> RatingDict:
    ratings: RatingDict = {}
    print("Enter ratings 1–5 (or blank to skip):")
    for it in items:
        while True:
            raw = input(f"Rate {it['title']} ({it['year']}) [1-5, blank to skip]: ").strip()
            if raw == "":
                break
            try:
                val = int(raw)
                if 1 <= val <= 5:
                    ratings[it["id"]] = float(val)
                    break
                else:
...
def main():
    print("RecoAI (Python-only, full datasets)")
    print("===================================")
    ...
    ratings = get_ratings_from_user(items)
    if not ratings:
        print("You did not rate anything. Exiting.")
        return

    algo = input("Algorithm (content/collaborative): ").strip().lower()
    if algo not in ("content", "collaborative"):
        print("Invalid algorithm. Defaulting to content.")
        algo = "content"

    if algo == "content":
        recs = content_based(items, ratings, top_n=6)
    else:
        recs = collaborative(items, ratings, users, top_n=6)
